...
if (window.location.hostname !== 'localhost' && 
    ...
    console.warn = () => {};
    console.info = () => {};
    // console.error necháme - chyby chceme vidět
}
...
function handleHashScroll() {
    ...
    if (!targetElement && (hash.startsWith('blog_') || hash.startsWith('blog'))) {
        let blogNumber = hash.replace(/^blog_?/, '');
        targetElement = document.querySelector(`.blog-card[data-id="blog_${blogNumber}"]`);
        
        if (!targetElement) {
            targetElement = document.querySelector(`.blog-card[data-id="${blogNumber}"]`);
        }
        
        if (!targetElement && blogScrollAttempts < MAX_BLOG_SCROLL_ATTEMPTS) {
            blogScrollAttempts++;
            setTimeout(() => handleHashScroll(), 500);
            return;
        }
        
        blogScrollAttempts = 0;
        
        if (targetElement) {
            setTimeout(() => {
                targetElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
                targetElement.classList.add('section-highlight');
                setTimeout(() => targetElement.classList.remove('section-highlight'), 2000);
            }, 100);
            return;
        }
    }
...
`;
    ...
    const shopContainer = document.getElementById("shop");
    if (shopContainer) {
        renderSkeletons(shopContainer, 8);
    }
