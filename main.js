...
import {cart, gdprConsent, setCartUpdateCallback, getCartTotalItems, getCartItems, generateCartWhatsAppMessage, generateCartWhatsAppMessageWithOrderNumber, openWhatsApp, initGDPR, acceptGDPR, declineGDPR, initShipping, setShipping, getShippingPrice, getShippingText, selectedShipping, removeFromCart as removeFromCartModule, changeCartItemQty, saveCart} from './cart.js';
import {renderSkeletons, renderFilters, applyFilters, showToast, setCartBadgeCallback, activeOverlayIdx, scrollStartPos, initInfiniteScroll, updateNavButtonsVisibility} from './ui.js';
    ...
    try {
        ...
        if (rating.count > 0) {
            ...
            for (let i = 0; i < Math.floor(rating.average); i++)
                starsHtml += '★';
            for (let i = 0; i < 5 - Math.floor(rating.average); i++)
                starsHtml += '☆';
...
function showMaintenanceMode() {
    ...
    document.querySelectorAll('.icon-btn, #floating-cart-btn, #chat-toggle-btn, .nav-link-btn, .f-btn, #search-input, .price-filter-btn').forEach(el => {
        if (el)
...
window.openProductModal = function(idx) {
    ...
    const shipText = product.ship || '';
const modalShip = document.getElementById('modal-ship');
if (shipText && shipText !== "Ne" && shipText.trim() !== "") {
    modalShip.innerHTML = `<strong>Dodatečné info:</strong><br>${shipText}`;
    ...
    if (modalBody) {
        ...
        if (addBtn)
            addBtn.onclick = () => showRatingForm(product.id, idx);
    }
...
function updateModalImage() {
    ...
    if (img) {
        img.style.opacity = '0';
        // Odstraníme ?w=300 z URL pro modal (použijeme originál)
        let originalUrl = images[currentModalImageIndex];
...
document.getElementById('shop')?.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.share-btn'))
        return;
    if (e.target.closest('.btn-cart-small'))
        return;
    if (e.target.closest('.btn-offer-small'))
        return;
    const card = e.target.closest('.product-card');
    if (card) {
        const idx = card.getAttribute('data-product-idx');
        if (idx)
            window.openProductModal(parseInt(idx));
    }
}
);
...
window.orderCart = () => {
    ...
    if (message) {
        createWoodShavings({
            target: document.querySelector('#cart-modal .btn-main')
        });
...
window.openModalImageFullscreen = function(event) {
    if (event)
    ...
    const currentIndex = product.imgs.findIndex(img => img === modalImg.src);

    if (typeof window.openImageGallery === 'function') {
        window.openImageGallery(product.imgs, currentIndex >= 0 ? currentIndex : 0, 'product');
    }
}
;

function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then( () => showToast("Obsah košíku zkopírován do schránky! 📋", "📋")).catch( () => showToast("Nepodařilo se zkopírovat.", "❌"));
}
...
window.switchCategoryBySwipe = (direction) => {
    ...
    if (newIndex >= filterButtons.length)
        newIndex = 0;
    ...
    if (targetButton) {
        ...
        setTimeout( () => targetButton.scrollIntoView({
            behavior: 'smooth',
            inline: 'center',
            block: 'nearest'
        }), 50);
...
function renderCart() {
    ...
    if (CONFIG.ShippingOptions && CONFIG.ShippingOptions.length > 0) {
        shippingOptionsHtml = `<div class="cart-shipping-section"><div class="cart-shipping-header" onclick="toggleShippingDropdown()"><span>🚚 <strong id="selected-shipping-text">${shippingText}</strong></span><span class="shipping-dropdown-arrow">▼</span></div><div class="shipping-dropdown" id="shipping-dropdown" style="display: none;">`;
        CONFIG.ShippingOptions.forEach(opt => {
            ...
            shippingOptionsHtml += `<div class="shipping-option ${isSelected ? 'selected' : ''}" onclick="selectShipping('${opt.id}')"><div class="shipping-option-info"><div class="shipping-option-name">${opt.name}</div><div class="shipping-option-price">${priceText}</div></div>${isSelected ? '<span class="shipping-option-check">✓</span>' : ''}</div>`;
        }
        );
    ...
    if (footer) {
        const discountHtml = `<div class="discount-section" style="margin-bottom: 15px;"><div style="display: flex; gap: 10px;"><input type="text" id="discount-code" placeholder="Slevový kód" style="flex: 1; padding: 12px; border-radius: 12px; border: 1px solid var(--border); background: var(--bg); color: var(--text);"><button id="apply-discount-btn" class="btn btn-sec" style="padding: 12px 20px;" onclick="window.applyDiscount()">Uplatnit</button></div><div id="discount-message" style="font-size: 12px; margin-top: 8px;"></div></div>`;
        footer.innerHTML = `${shippingOptionsHtml}${discountHtml}<div class="cart-total"><span>Celkem včetně dopravy:</span><span id="cart-total-price-display">${totalDisplay}</span></div><button class="btn btn-main btn-pulse" onclick="orderCart()">${CONFIG.Cart_Button_Text}</button>`;
    }
}
...
function applyTheme() {
    ...
    if (savedTheme === 'light') {
        ...
        const themeColor = document.querySelector('meta[name="theme-color"]');
        if (themeColor)
            themeColor.setAttribute('content', '#f5f2ed');
    } else {
        ...
        const themeColor = document.querySelector('meta[name="theme-color"]');
        if (themeColor)
            themeColor.setAttribute('content', '#0d0d0d');
    }
}
...
window.showProfileTab = function(tab) {
    ...
    if (tab === 'profile') {
        ...
        ordersBtn.style.color = 'var(--text-dim)';
    } else if (tab === 'orders') {
        ...
        profileBtn.style.color = 'var(--text-dim)';
        const now = Date.now();
        const hasValidCache = cachedUserOrders && (now - lastOrdersFetch) < CACHE_TTL;
        if (!hasValidCache)
            window.loadUserOrders(true);
        else {
            const container = document.getElementById('orders-list-content');
            const loaderOverlay = document.getElementById('orders-loader-overlay');
            if (container && loaderOverlay) {
                loaderOverlay.style.display = 'none';
                container.style.display = 'block';
                renderUserOrders(cachedUserOrders, container);
            }
        }
    }
}
;
window.loadUserOrders = async function(forceRefresh=false) {
    const userId = localStorage.getItem('woodisek_userId');
    if (!userId)
        return;
    const now = Date.now();
    ...
    if (!container || !loaderOverlay)
        return;
    const loaderSteps = document.querySelectorAll('#orders-loader-overlay .loader-step');
    const loaderText = document.querySelector('#orders-loader-overlay .loader-text');
    const loaderSubtext = document.querySelector('#orders-loader-overlay .loader-subtext');
    function updateLoaderStep(stepIndex, status) {
        const step = loaderSteps[stepIndex - 1];
        if (step) {
            step.classList.remove('active', 'completed');
            if (status === 'active') {
                step.classList.add('active');
                step.querySelector('.step-dot').style.background = 'var(--accent)';
            } else if (status === 'completed') {
                step.classList.add('completed');
                step.querySelector('.step-dot').style.background = '#5b7c5d';
            }
        }
    }
    function resetLoaderSteps() {
        loaderSteps.forEach(step => {
            step.classList.remove('active', 'completed');
            step.querySelector('.step-dot').style.background = 'var(--text-dim)';
        }
        );
    }
    if (!forceRefresh && cachedUserOrders && (now - lastOrdersFetch) < CACHE_TTL) {
        loaderOverlay.style.display = 'none';
        container.style.display = 'block';
        renderUserOrders(cachedUserOrders, container);
        return;
    ...
    loaderText.innerText = 'Načítám objednávky';
    loaderSubtext.innerText = 'Připojuji se k serveru';
    let step1Done = false;
    setTimeout( () => {
        if (loaderOverlay.style.display === 'flex' && !step1Done) {
            updateLoaderStep(1, 'completed');
            updateLoaderStep(2, 'active');
            loaderSubtext.innerText = 'Stahuji vaše objednávky';
            step1Done = true;
        }
    }
    , 400);
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=getOrders&userId=${encodeURIComponent(userId)}`);
        const data = await response.json();
        updateLoaderStep(2, 'completed');
        updateLoaderStep(3, 'active');
        loaderSubtext.innerText = 'Zpracovávám data';
        await new Promise(resolve => setTimeout(resolve, 250));
        if (data.success && data.orders && data.orders.length > 0) {
            cachedUserOrders = data.orders;
            lastOrdersFetch = now;
            renderUserOrders(cachedUserOrders, container);
        } else {
            container.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--text-dim);">📦 Zatím žádné objednávky</div>';
        }
        updateLoaderStep(3, 'completed');
        loaderText.innerText = 'Hotovo!';
        loaderSubtext.innerText = 'Objednávky jsou načteny';
        setTimeout( () => {
            loaderOverlay.style.display = 'none';
            container.style.display = 'block';
        }
        ...
        loaderText.innerText = 'Chyba připojení';
        loaderSubtext.innerText = 'Zkuste to prosím znovu';
        setTimeout( () => {
            loaderOverlay.style.display = 'none';
            container.style.display = 'block';
            container.innerHTML = '<div style="text-align: center; padding: 40px; color: #dc2626;">❌ Nepodařilo se načíst objednávky<br><span style="font-size: 12px; margin-top: 10px; display: inline-block;">Zkuste obnovit stránku</span></div>';
        }
        , 1200);
    }
}
...
function renderUserProfile(user) {
    ...
    document.getElementById('user-forgot-form').style.display = 'none';
    document.getElementById('user-profile').style.display = 'block';
    setTimeout( () => window.checkNewsletterStatus(), 200);
}
function renderUserOrders(orders, container) {
    ...
    orders.forEach(order => {
        ...
        if (statusLower === 'nová')
            statusColor = '#f59e0b';
        if (statusLower === 'odeslána')
            statusColor = '#10b981';
        if (statusLower === 'zrušena')
            statusColor = '#ef4444';
        const orderDate = new Date(order.date).toLocaleDateString('cs-CZ');
        let itemsHtml = '';
        if (order.items && order.items.length > 0) {
            itemsHtml = '<div style="margin-top: 15px; border-top: 1px solid var(--border); padding-top: 10px; display: flex; flex-direction: column; gap: 12px;">';
            order.items.forEach(item => {
                itemsHtml += `<div style="display: flex; gap: 12px; align-items: center; cursor: pointer;" onclick="window.showProductsAndResetFilter ? window.showProductsAndResetFilter() : window.showProducts();"><img src="${item.img}" style="width: 70px; height: 70px; border-radius: 6px; object-fit: cover; border: 1px solid var(--border); flex-shrink: 0;"><div style="flex: 1; min-width: 0;"><div style="font-size: 13px; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${item.name}</div><div style="font-size: 11px; color: var(--text-dim);">Kód: <strong>${item.id}</strong> | ${item.qty}x ${item.price}</div></div></div>`;
            }
            );
            itemsHtml += '</div>';
        }
        html += `<div style="margin-bottom: 25px;"><div style="font-size: 11px; color: var(--text-dim); font-weight: 700; margin-bottom: 6px; margin-left: 4px; text-transform: uppercase; letter-spacing: 0.5px;">${orderDate}</div><div style="background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 15px; position: relative;"><div style="display: flex; justify-content: space-between; align-items: flex-start;"><div><div style="font-weight: 700; font-size: 14px; color: var(--text);">📦 ${order.orderId}</div></div><div style="text-align: right;"><div style="padding: 3px 10px; border-radius: 10px; font-size: 10px; font-weight: 800; background: ${statusColor}20; color: ${statusColor}; border: 1px solid ${statusColor}40; display: inline-block; text-transform: uppercase;">${order.status}</div></div></div><div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;"><div style="font-size: 13px; color: var(--text-dim);">🚚 ${order.shipping || 'Doprava'}</div><div style="font-size: 15px; font-weight: 700; color: var(--text);">${order.total} Kč</div></div>${itemsHtml}</div></div>`;
    }
    );
    container.innerHTML = html || '<p style="text-align:center; color:var(--text-dim); padding: 20px;">Zatím žádné objednávky.</p>';
}
...
document.getElementById('shop')?.addEventListener('click', (e) => {
    ...
    case 'add-to-cart':
        if (idx)
            window.addToCart(parseInt(idx), e);
        break;
    case 'add-to-wishlist':   // 👈 PŘIDEJ TENTO CASE
        if (idx) {
            import('./wishlist.js').then(w => {
                const product = allProducts.find(p => p.idx === parseInt(idx));
                if (product) w.addToWishlist(product, e);
            });
        }
        break;
    case 'order-product':
        if (idx)
            window.orderProduct(parseInt(idx), e);
        break;
    case 'show-details':
        if (idx)
            window.toggleOver(parseInt(idx), true);
        break;
    case 'close-details':
        if (idx)
            window.toggleOver(parseInt(idx), false);
        break;
    case 'open-offer':
        if (idx)
            window.openOffer(parseInt(idx));
        break;
    case 'open-gallery':
        if (idx)
            window.openGal(parseInt(idx));
        break;
    case 'share-product':
        if (productId && productName)
            window.shareProduct(productId, productName, e);
        break;
    case 'prev-image':
        if (idx)
            window.chImg(parseInt(idx), -1, e);
        e.stopPropagation();
        break;
    case 'next-image':
        if (idx)
...
function applyTextsFromSettings() {
    ...
    if (logoText)
        logoText.innerText = CONFIG.Nazev_Firmy;
    ...
    if (cartTitle)
        cartTitle.innerHTML = CONFIG.Cart_Title;
    if (aboutTitle)
        aboutTitle.innerText = CONFIG.About_Titulek;
    const chatBtn = document.getElementById('chat-toggle-btn');
    if (chatBtn)
    ...
    if (CONFIG.Meta_Description) {
        ...
        if (ogDesc)
            ogDesc.setAttribute('content', CONFIG.Meta_Description);
        const twitterDesc = document.querySelector('meta[name="twitter:description"]');
        if (twitterDesc)
            twitterDesc.setAttribute('content', CONFIG.Meta_Description.substring(0, 200));
    }
    ...
    if (gdprText && CONFIG.GDPR_Banner_Text) {
        let fixedText = CONFIG.GDPR_Banner_Text.replace(/""/g, '"');
        gdprText.innerHTML = fixedText;
    }
    const reviewsPanelText = document.getElementById('reviews-panel-text');
    if (reviewsPanelText && CONFIG.Reviews_Panel_Text)
        reviewsPanelText.innerHTML = CONFIG.Reviews_Panel_Text;
    const reviewsBtnText = document.getElementById('reviews-btn-text');
    if (reviewsBtnText && CONFIG.Reviews_Button_Text)
        reviewsBtnText.innerText = CONFIG.Reviews_Button_Text;
    const blogTitleText = document.getElementById('blog-title-text');
    if (blogTitleText && CONFIG.Blog_Title)
        blogTitleText.innerHTML = CONFIG.Blog_Title;
    const offerBtnText = document.getElementById('offer-btn-text');
    if (offerBtnText && CONFIG.Offer_Button_Text)
        offerBtnText.innerText = CONFIG.Offer_Button_Text;
    const productCountEl = document.getElementById("product-count");
    if (productCountEl && allProducts)
        productCountEl.innerHTML = `${allProducts.length} ${CONFIG.Product_Count_Text || "🪵 produktů"}`;
    if (customTitle)
        customTitle.innerText = CONFIG.Zakazkova_Tvorba_Titulek;
    if (customText)
        customText.innerText = CONFIG.Zakazkova_Tvorba_Text;
    if (customBtn) {
...
if (isIOS) {
    window.location.href = `whatsapp://send?phone=${CONFIG.WhatsApp_Cislo_Zakazka}&text=${message}`;
    setTimeout(() => {
        if (!document.hidden) {
            window.open(`https://wa.me/${CONFIG.WhatsApp_Cislo_Zakazka}?text=${message}`, '_blank');
        }
    }, 500);
} else {
    window.open(`https://wa.me/${CONFIG.WhatsApp_Cislo_Zakazka}?text=${message}`, '_blank');
    ...
    if (footer)
        footer.innerText = CONFIG.Footer_Text;
    window.openQrZoom = function() {
        ...
        if (gal && galImg) {
            galImg.src = CONFIG.QR_CODE_URL;
            gal.style.display = "flex";
            galImg.style.pointerEvents = "none";
            if (galPrev)
...
function addQuickTapAnimation(selector) {
    document.querySelectorAll(selector).forEach(btn => {
        if (btn.hasAttribute('data-quick-animation'))
            return;
...
function initQuickAnimations() {
    ['.btn-offer-small', '.btn-cart-small', '.btn.btn-main', '.btn.btn-sec', '#modal-order-btn', '#modal-cart-btn', '#modal-offer-btn', '#custom-wa-btn', '.btn.btn-main.btn-pulse', '.share-cart-btn'].forEach(selector => addQuickTapAnimation(selector));
}
document.addEventListener('DOMContentLoaded', initQuickAnimations);
new MutationObserver( () => initQuickAnimations()).observe(document.body, {
    childList: true,
    subtree: true
});
async function init() {
    if (window.skipNormalInit) return;
    ...
    applyTheme();
    showGDPRBannerIfNeeded();

    
    
    // Těžké věci se načtou až po vykreslení
    setTimeout(async () => {
        await loadBlogPosts();
        renderBlogPosts();
        await loadSettings();
    ...
    initShipping();
    initPortfolio();
    initGlobalSwipeDetection( (direction) => window.switchCategoryBySwipe(direction));
    initSocialProof();
    if (CONFIG.Enable_Shop === false) {
        ...
        if (priceFilterBtn)
            priceFilterBtn.style.display = "flex";
        const productCountEl = document.getElementById("product-count");
        if (productCountEl) {
    ...
    setTimeout( () => handleHashScroll(), 500);
        }, 50);  // 👈 TOTO MUSÍ BÝT - uzavření setTimeout
}
document.addEventListener('DOMContentLoaded', () => {
    init();
    window.addEventListener('load', () => setTimeout( () => handleHashScroll(), 1500));
}
);
...
function initAnimatedPlaceholder() {
    ...
    if (!searchInput) {
        setTimeout(initAnimatedPlaceholder, 500);
        return;
    }
    const phrases = ["Co si vyrobíme ??", "Téměř vše lze personalizovat !!", "Zakázková výroba ??", "Výrobky lze vyrobit i větší !!", "Zboží skladem - ihned k odeslání !!", "Hledáte dárek ??", "Přívěsek na klíče ??", "Jmenovka na dveře ??", "Dřevěný obraz ??", "Podtácky na víno ??", "Fotka do dřeva ??", "Dárek pro blízké ??", "Gravírované pero ??", "Cedule na chalupu ??", "Doprava Zásilkovnou ??", "Vánoční dekorace ??", ];
    let phraseIndex = 0;
    let charIndex = 0;
    ...
    searchInput.placeholder = "Hledej dřevěné dárky...";
...
function initStatsFlip() {
    const cards = document.querySelectorAll('.stats-flip-card');
    if (cards.length === 0)
        return;
    let currentIndex = 0;
...
if (document.readyState === 'loading')
    document.addEventListener('DOMContentLoaded', initStatsFlip);
else
    initStatsFlip();
...
window.initLoginTurnstile = function() {
    ...
    const placeholder = document.getElementById('login-recaptcha-placeholder');
    ...
    if (turnstileWidgets.login) {
        turnstile.reset(turnstileWidgets.login);
        return;
    }
    turnstileWidgets.login = turnstile.render(container, {
        sitekey: '0x4AAAAAAC6qi8ceRKAigUhk',
...
;
window.initRegisterTurnstile = function() {
    const container = document.getElementById('register-turnstile-container');
    const placeholder = document.getElementById('register-recaptcha-placeholder');
    if (!container || !placeholder)
        return;
    if (container.parentNode !== placeholder) {
        placeholder.innerHTML = '';
    ...
    if (turnstileWidgets.register) {
        turnstile.reset(turnstileWidgets.register);
        return;
    }
    turnstileWidgets.register = turnstile.render(container, {
        sitekey: '0x4AAAAAAC6qi8ceRKAigUhk',
        callback: function(token) {
            container.setAttribute('data-turnstile-token', token);
        },
...
;
window.initForgotTurnstile = function() {
    const container = document.getElementById('forgot-turnstile-container');
    const placeholder = document.getElementById('forgot-recaptcha-placeholder');
    if (!container || !placeholder)
        return;
    if (container.parentNode !== placeholder) {
        placeholder.innerHTML = '';
    ...
    if (turnstileWidgets.forgot) {
        turnstile.reset(turnstileWidgets.forgot);
        return;
    }
    turnstileWidgets.forgot = turnstile.render(container, {
        sitekey: '0x4AAAAAAC6qi8ceRKAigUhk',
        callback: function(token) {
            container.setAttribute('data-turnstile-token', token);
        },
...
function getLoginTurnstileToken() {
    const container = document.getElementById('login-turnstile-container');
    return container?.getAttribute('data-turnstile-token') || '';
}
function getRegisterTurnstileToken() {
    const container = document.getElementById('register-turnstile-container');
    return container?.getAttribute('data-turnstile-token') || '';
}
function getForgotTurnstileToken() {
    const container = document.getElementById('forgot-turnstile-container');
    return container?.getAttribute('data-turnstile-token') || '';
}
...
window.showLoginForm = function() {
    document.querySelectorAll('.global-overlay').forEach(overlay => overlay.style.display = 'none');
    document.getElementById('user-login-form').style.display = 'block';
    document.getElementById('user-register-form').style.display = 'none';
    document.getElementById('user-forgot-form').style.display = 'none';
    document.getElementById('user-profile').style.display = 'none';
    setTimeout( () => window.initLoginTurnstile(), 100);
}
;
window.showRegisterForm = function() {
    ...
    document.getElementById('user-forgot-form').style.display = 'none';
    document.getElementById('user-profile').style.display = 'none';
    setTimeout( () => window.initRegisterTurnstile(), 100);
}
;
window.showForgotPassword = function() {
    ...
    document.getElementById('user-login-form').style.display = 'none';
    document.getElementById('user-register-form').style.display = 'none';
    document.getElementById('user-forgot-form').style.display = 'block';
    document.getElementById('user-profile').style.display = 'none';
    setTimeout( () => window.initForgotTurnstile(), 100);
}
;
window.loginUser = async function() {
    ...
    if (!turnstileToken || turnstileToken.length === 0) {
        import('./ui.js').then(ui => ui.showToast("Potvrďte, že jste člověk", "🤖"));
        return;
    }
    const loaderOverlay = document.getElementById('login-loader-overlay');
    const loaderText = document.getElementById('login-loader-text');
    const loaderSubtext = document.getElementById('login-loader-subtext');
    const loaderSteps = document.querySelectorAll('#login-loader-steps .loader-step');
    function updateLoaderStep(stepIndex, status) {
        const step = loaderSteps[stepIndex - 1];
        if (step) {
            step.classList.remove('active', 'completed');
    ...
    if (!email || !password) {
        import('./ui.js').then(ui => ui.showToast("Zadejte email a heslo", "❌"));
        return;
    }
    loaderOverlay.style.display = 'flex';
    resetLoaderSteps();
    updateLoaderStep(1, 'active');
    loaderText.innerText = 'Přihlašování...';
    loaderSubtext.innerText = 'Odesílám požadavek na server';
    let step1Completed = false;
    setTimeout( () => {
        if (loaderOverlay.style.display === 'flex' && !step1Completed) {
            updateLoaderStep(1, 'completed');
            updateLoaderStep(2, 'active');
            loaderSubtext.innerText = 'Ověřuji přihlašovací údaje';
            step1Completed = true;
        }
    }
    , 600);
    ...
    let userData = null;
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=login&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&turnstile=${turnstileToken}`);
        const data = await response.json();
        updateLoaderStep(2, 'completed');
        updateLoaderStep(3, 'active');
        loaderSubtext.innerText = 'Načítám váš profil';
        if (data.success) {
            resetLoginTurnstile();
            renderCart();
            localStorage.setItem('woodisek_token', data.token);
            localStorage.setItem('woodisek_userId', data.userId);
            localStorage.setItem('woodisek_user', JSON.stringify(data.user));
            success = true;
            userData = data;
        } else {
            setTimeout( () => {
                loaderOverlay.style.display = 'none';
                import('./ui.js').then(ui => ui.showToast(data.error || "Špatné jméno nebo heslo", "❌"));
            }
            , 500);
            return;
        }
    ...
    if (success && userData) {
        loaderText.innerText = 'Načítám profil...';
        loaderSubtext.innerText = 'Připravuji uživatelské prostředí';
        await window.loadUserProfile(userData.userId, true);
        setTimeout( () => {
            loaderOverlay.style.display = 'none';
            if (loginForm)
            ...
            import('./ui.js').then(ui => ui.showToast("Vítejte zpět! 🪵", "👋"));
        }
        , 400);
    }
}
...
;
window.showUserSection = async function() {
    ...
    if (priceFilterToggle)
        priceFilterToggle.style.display = 'none';
    if (cartBtn)
    ...
    document.getElementById('user-profile').style.display = 'none';
    setTimeout( () => {
        if (typeof window.initLoginTurnstile === 'function')
            window.initLoginTurnstile();
    }
    , 100);
    if (typeof resetLoginTurnstile === 'function')
        resetLoginTurnstile();
    if (typeof resetRegisterTurnstile === 'function')
        resetRegisterTurnstile();
    if (typeof resetForgotTurnstile === 'function')
    ...
    if (userId) {
        const now = Date.now();
        const hasValidCache = cachedUserProfile && (now - lastProfileFetch) < CACHE_TTL;
        if (hasValidCache) {
            ...
            const loginLoaderOverlay = document.getElementById('login-loader-overlay');
            const loginForm = document.getElementById('user-login-form');
            const loaderText = document.getElementById('login-loader-text');
            const loaderSubtext = document.getElementById('login-loader-subtext');
            if (loginLoaderOverlay && loginForm) {
                if (loaderText)
                    loaderText.innerText = 'Načítám profil...';
                if (loaderSubtext)
                    loaderSubtext.innerText = 'Připravuji vaše uživatelské prostředí';
                loginLoaderOverlay.style.display = 'flex';
                await window.loadUserProfile(userId, false);
                loginLoaderOverlay.style.display = 'none';
                if (loginForm)
                    loginForm.style.display = 'none';
                const profileSection = document.getElementById('user-profile');
                if (profileSection)
...
window.registerUser = async function() {
    ...
    const turnstileToken = getRegisterTurnstileToken();
    if (!turnstileToken || turnstileToken.length === 0) {
        import('./ui.js').then(ui => ui.showToast("Potvrďte, že jste člověk", "🤖"));
        return;
    }
    const loaderOverlay = document.getElementById('register-loader-overlay');
    const loaderText = document.getElementById('register-loader-text');
    const loaderSubtext = document.getElementById('register-loader-subtext');
    const loaderSteps = document.querySelectorAll('#register-loader-steps .loader-step');
    function updateLoaderStep(stepIndex, status) {
        const step = loaderSteps[stepIndex - 1];
        if (step) {
            step.classList.remove('active', 'completed');
    ...
    if (!email || !name || !password) {
        import('./ui.js').then(ui => ui.showToast("Vyplňte všechna pole", "❌"));
        return;
    }
    ...
    resetLoaderSteps();
    updateLoaderStep(1, 'active');
    loaderText.innerText = 'Registrace...';
    loaderSubtext.innerText = 'Odesílám registrační údaje';
    setTimeout( () => {
        if (loaderOverlay.style.display === 'flex') {
            updateLoaderStep(1, 'completed');
            updateLoaderStep(2, 'active');
            loaderSubtext.innerText = 'Vytvářím účet';
        }
    }
    , 600);
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=register&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&name=${encodeURIComponent(name)}&turnstile=${turnstileToken}`);
        const data = await response.json();
        updateLoaderStep(2, 'completed');
        updateLoaderStep(3, 'active');
        loaderSubtext.innerText = 'Dokončuji registraci';
        if (data.success) {
            resetRegisterTurnstile();
            updateLoaderStep(3, 'completed');
            loaderText.innerText = 'Hotovo!';
            loaderSubtext.innerText = 'Registrace proběhla úspěšně';
            setTimeout( () => {
                loaderOverlay.style.display = 'none';
            ...
            setTimeout( () => {
                loaderOverlay.style.display = 'none';
                import('./ui.js').then(ui => ui.showToast(data.error || "Registrace selhala", "❌"));
            }
            , 500);
        }
    } catch (err) {
...
window.forgotPassword = async function() {
    ...
    const turnstileToken = getForgotTurnstileToken();
    if (!turnstileToken || turnstileToken.length === 0) {
        import('./ui.js').then(ui => ui.showToast("Potvrďte, že jste člověk", "🤖"));
        return;
    }
    const loaderOverlay = document.getElementById('forgot-loader-overlay');
    const loaderText = document.getElementById('forgot-loader-text');
    const loaderSubtext = document.getElementById('forgot-loader-subtext');
    const loaderSteps = document.querySelectorAll('#forgot-loader-steps .loader-step');
    function updateLoaderStep(stepIndex, status) {
        const step = loaderSteps[stepIndex - 1];
        if (step) {
            step.classList.remove('active', 'completed');
    ...
    if (!email) {
        import('./ui.js').then(ui => ui.showToast("Zadejte email", "❌"));
        return;
    }
    loaderOverlay.style.display = 'flex';
    resetLoaderSteps();
    updateLoaderStep(1, 'active');
    loaderText.innerText = 'Odesílám...';
    loaderSubtext.innerText = 'Odesílám požadavek na server';
    setTimeout( () => {
        if (loaderOverlay.style.display === 'flex') {
            updateLoaderStep(1, 'completed');
            updateLoaderStep(2, 'active');
            loaderSubtext.innerText = 'Ověřuji email';
        }
    }
    , 600);
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=forgotPassword&email=${encodeURIComponent(email)}&turnstile=${turnstileToken}`);
        const data = await response.json();
        updateLoaderStep(2, 'completed');
        updateLoaderStep(3, 'active');
        loaderSubtext.innerText = 'Odesílám odkaz';
        setTimeout( () => {
            loaderOverlay.style.display = 'none';
            import('./ui.js').then(ui => ui.showToast("Pokud zadaný e-mail existuje, byl na něj odeslán odkaz pro obnovení hesla.", "📧"));
            window.showLoginForm();
        }
        , 800);
    } catch (err) {
...
window.updateUserProfile = async function() {
    const userId = localStorage.getItem('woodisek_userId');
    if (!userId) {
        import('./ui.js').then(ui => ui.showToast("Nejste přihlášeni", "❌"));
        return;
    }
    const name = document.getElementById('profile-name').value;
    const phone = document.getElementById('profile-phone').value;
    const address = document.getElementById('profile-address').value;
    import('./ui.js').then(ui => ui.showToast("Ukládám...", "⏳"));
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=updateUser&userId=${encodeURIComponent(userId)}&name=${encodeURIComponent(name)}&phone=${encodeURIComponent(phone)}&address=${encodeURIComponent(address)}`);
        const data = await response.json();
        if (data.success) {
            import('./ui.js').then(ui => ui.showToast("Profil aktualizován", "✅"));
            if (typeof window.checkNewsletterStatus === 'function')
                window.checkNewsletterStatus(true);
            const user = JSON.parse(localStorage.getItem('woodisek_user') || '{}');
            user.name = name;
            user.phone = phone;
            user.address = address;
            localStorage.setItem('woodisek_user', JSON.stringify(user));
            if (typeof cachedUserProfile !== 'undefined')
                cachedUserProfile = null;
            if (typeof lastProfileFetch !== 'undefined')
                lastProfileFetch = 0;
            ...
            import('./ui.js').then(ui => ui.showToast(data.error || "Chyba uložení", "❌"));
        }
    } catch (err) {
        import('./ui.js').then(ui => ui.showToast("Chyba připojení", "❌"));
    }
...
window.logoutUser = function() {
    ...
    sessionStorage.removeItem('woodisek_user_address');
    cachedUserProfile = null;
    cachedUserOrders = null;
    renderCart();
    updateCartBadge();
    const userSection = document.getElementById('user-section');
    if (userSection && userSection.getAttribute('data-initialized') === 'true') {
        document.getElementById('user-login-form').style.display = 'block';
        document.getElementById('user-register-form').style.display = 'none';
        document.getElementById('user-forgot-form').style.display = 'none';
        document.getElementById('user-profile').style.display = 'none';
        ...
        , 100);
        if (typeof resetLoginRecaptcha === 'function')
            resetLoginTurnstile();
        if (typeof resetRegisterRecaptcha === 'function')
            resetRegisterTurnstile();
        if (typeof resetForgotRecaptcha === 'function')
            resetForgotTurnstile();
    } else {
        window.showUserSection();
    }
    setTimeout( () => {
        if (typeof initLoginTurnstile === 'function')
            initLoginTurnstile();
        if (typeof initRegisterTurnstile === 'function')
            initRegisterTurnstile();
        if (typeof initForgotTurnstile === 'function')
            initForgotTurnstile();
    }
    , 150);
    import('./ui.js').then(ui => ui.showToast("Odhlášení proběhlo úspěšně", "✅"));
}
;
window.orderCart = async () => {
    ...
    if (!userAddress && user.address && user.address.trim() !== "")
        userAddress = user.address;
    if (!userEmail && user.email && user.email.trim() !== "")
...
window.showResetPasswordForm = function(token) {
    ...
    if (floatingCartBtn)
        floatingCartBtn.style.display = 'flex';
    if (reviewsSidebar)
        reviewsSidebar.style.display = 'none';
    if (cartIcon)
...
window.submitResetPassword = async function() {
    ...
    if (!token)
        token = sessionStorage.getItem('woodisek_reset_token');
...
window.checkNewsletterStatus = async function(forceRefresh=false) {
    const user = JSON.parse(localStorage.getItem('woodisek_user') || '{}');
    const email = user.email;
    if (!email)
        return;
    const now = Date.now();
    if (!forceRefresh && cachedNewsletterStatus !== null && (now - lastNewsletterFetch) < CACHE_TTL) {
        applyNewsletterButtonState(cachedNewsletterStatus);
        return;
    }
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=newsletterStatus&email=${encodeURIComponent(email)}`);
        const data = await response.json();
        cachedNewsletterStatus = data.subscribed === true;
        lastNewsletterFetch = now;
        applyNewsletterButtonState(cachedNewsletterStatus);
    } catch (err) {
        console.error("Chyba při kontrole newsletteru:", err);
    }
}
;
function applyNewsletterButtonState(isSubscribed) {
...
window.subscribeNewsletter = async function() {
    ...
    import('./ui.js').then(ui => ui.showToast("Přihlašuji k odběru...", "⏳"));
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=newsletter&email=${encodeURIComponent(email)}`);
        const data = await response.json();
        if (data.success) {
            import('./ui.js').then(ui => ui.showToast("Přihlášeni k odběru novinek! ✅", "📧"));
            window.checkNewsletterStatus(true);
        } else {
            import('./ui.js').then(ui => ui.showToast(data.error || "Chyba", "❌"));
        }
    } catch (err) {
        import('./ui.js').then(ui => ui.showToast("Chyba připojení", "❌"));
    }
...
;
window.unsubscribeNewsletter = async function() {
    const user = JSON.parse(localStorage.getItem('woodisek_user') || '{}');
    const email = user.email;
    if (!email) {
        import('./ui.js').then(ui => ui.showToast("Nejste přihlášeni", "❌"));
    ...
    import('./ui.js').then(ui => ui.showToast("Odhlašuji z odběru...", "⏳"));
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=newsletterUnsub&email=${encodeURIComponent(email)}`);
        const data = await response.json();
        if (data.success) {
            import('./ui.js').then(ui => ui.showToast("Odhlášeni z odběru ✅", "📧"));
            window.checkNewsletterStatus(true);
        } else {
            import('./ui.js').then(ui => ui.showToast(data.error || "Chyba", "❌"));
        }
...
;
window.submitRating = async function(productId, idx) {
    const userId = localStorage.getItem('woodisek_userId');
    if (!userId) {
        import('./ui.js').then(ui => ui.showToast("Nejste přihlášeni", "❌"));
        return;
    }
    const rating = parseInt(document.getElementById(`selected-rating-${idx}`).value) || 5;
    const comment = document.getElementById(`rating-comment-${idx}`).value;
    import('./ui.js').then(ui => ui.showToast("Odesílám hodnocení...", "⏳"));
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=addRatingIfNotRated&productId=${encodeURIComponent(productId)}&userId=${encodeURIComponent(userId)}&rating=${rating}&comment=${encodeURIComponent(comment)}`);
        const data = await response.json();
        if (data.success) {
            clearRatingCache(productId);
            import('./ui.js').then(ui => ui.showToast("Děkuji za hodnocení! ✅", "⭐"));
            window.loadRatingsToModal(productId, idx, true);
        } else {
            import('./ui.js').then(ui => ui.showToast(data.error || "Chyba při ukládání", "❌"));
        }
    } catch (err) {
        import('./ui.js').then(ui => ui.showToast("Chyba připojení", "❌"));
    }
...
window.applyDiscount = async function() {
    ...
    if (!code) {
        if (messageDiv)
            messageDiv.innerHTML = '<span style="color: #dc2626;">❌ Zadejte slevový kód</span>';
        return;
    }
    const cartItems = getCartItems();
    let cartTotal = 0;
    ...
    cartItems.forEach(item => {
        ...
        cartTotal += itemTotal;
        if (product && product.sale && product.sale !== "0")
            hasSaleItems = true;
        else
            nonSaleTotal += itemTotal;
    }
    );
    if (hasSaleItems && nonSaleTotal === 0) {
        if (messageDiv)
            messageDiv.innerHTML = '<span style="color: #dc2626;">❌ Slevový kód nelze použít na produkty, které jsou již ve slevě</span>';
        return;
    }
    try {
        const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=validateDiscount&code=${encodeURIComponent(code)}&total=${cartTotal}&nonSaleTotal=${nonSaleTotal}&hasSaleItems=${hasSaleItems}`);
        const data = await response.json();
        if (data.valid) {
            window.appliedDiscount = data;
            if (messageDiv)
                messageDiv.innerHTML = `<span style="color: #10b981;">✅ Sleva uplatněna: ${data.type === 'percent' ? data.value + '%' : data.value + ' Kč'}</span>`;
            if (codeInput)
...
;
window.getDiscountedTotal = function() {
    const cartItems = getCartItems();
    let cartTotal = 0;
    cartItems.forEach(item => {
        const priceNum = parseInt(item.price.replace(/[^\d]/g, ''), 10) || 0;
        cartTotal += priceNum * item.qty;
    }
    );
    if (window.appliedDiscount && window.appliedDiscount.valid)
        return cartTotal - window.appliedDiscount.discountAmount;
    return cartTotal;
}
;
function setupBannerLinks() {
    const bannerText = document.querySelector('.welcome-banner-text');
    if (!bannerText)
        return;
    bannerText.querySelectorAll('strong').forEach(link => {
        link.style.cursor = 'pointer';
        link.addEventListener('click', (e) => {
            e.stopPropagation();
            const text = link.innerText.toLowerCase();
            if (text.includes('přihlas') || text.includes('přihlaš'))
                window.showLoginForm();
            else if (text.includes('registruj') || text.includes('zaregistruj'))
...
window.applyAllFilters = window.applyAllFilters || function() {
...
// Po načtení stránky schovat tlačítko nástrojů (není na hlavní stránce)
//document.addEventListener('DOMContentLoaded', function() {
//    const appsBtn = document.querySelector('.icon-btn[onclick="window.showApps()"]');
 //   if (appsBtn) appsBtn.style.display = 'none';
//});

// Oprava tlačítka aplikací
setTimeout(() => {
    const appsBtn = document.querySelector('.icon-btn[onclick="window.showApps()"]');
    if (appsBtn) {
        console.log('🔧 Fixing apps button');
        appsBtn.onclick = (e) => {
            e.preventDefault();
            window.showApps();
        };
