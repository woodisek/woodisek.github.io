...
// ============================================================
// gallery.js - Lightbox galerie, portfolio slider, univerzální galerie
// ============================================================
...
// STAV
// ============================================================
let activeGalProdIdx = null;      // index produktu v galerii, null = portfolio
let portIdx = 0;                  // aktuální index v portfoliu
let isSwiping = false;
let touchStartX = 0;
let touchStartY = 0;

// ============================================================
// UNIVERZÁLNÍ GALERIE PRO PORTFOLIO A BLOG
// ============================================================
let activeGalleryImages = [];     // pole URL obrázků
let activeGalleryIndex = 0;       // aktuální index
let activeGalleryType = null;     // 'portfolio', 'blog', 'product'

// ============================================================
// PRODUKTOVÁ GALERIE (původní)
// ============================================================
window.openGal = function(idx) {
    if (isSwiping) return;
    
    if (!allProducts || allProducts.length === 0) {
        console.warn("Data ještě nejsou načtena");
        return;
    }
...
// ============================================================
// PORTFOLIO SLIDER (na stránce)
// ============================================================
export function initPortfolio() {
    console.log('🖼️ initPortfolio volán, portfolioImages:', portfolioImages);
    ...
    if (portfolioImages && portfolioImages.length > 0) {
        ...
        console.log('🖼️ Portfolio inicializováno s', portfolioImages.length, 'obrázky');
    } else {
        portfolioContainer.style.display = "none";
        console.log('🖼️ Žádné portfolio obrázky k zobrazení');
    }
}
...
function updatePortfolioImage() {
    ...
    img.onload = () => {
        ...
        img.style.opacity = "1";
        console.log('✅ Portfolio obrázek načten:', imgUrl);
    };
    
    img.onerror = () => {
        console.error('❌ Portfolio obrázek se nepodařilo načíst:', imgUrl);
        spin.style.display = "none";
...
window.navPort = function(step) {
    ...
    // Pokud je galerie otevřená a je to portfolio, aktualizujeme ji
    const gal = document.getElementById("gal");
...
// ============================================================
// PORTFOLIO FULLSCREEN GALERIE (univerzální)
// ============================================================
window.openPortfolioZoom = function() {
    if (!portfolioImages || portfolioImages.length === 0) return;
    
    // Nastavíme univerzální galerii
...
// ============================================================
// UNIVERZÁLNÍ NAVIGACE V GALERII
// ============================================================
window.navGal = function(step, e) {
    if (e) e.stopPropagation();
    
    // 1. Univerzální galerie (portfolio nebo blog)
    if (activeGalleryImages && activeGalleryImages.length > 0) {
        ...
        // Aktualizujeme portIdx pokud je to portfolio
    ...
    // 2. Portfolio (stará logika – pro zpětnou kompatibilitu)
...
// ============================================================
// GLOBÁLNÍ SWIPE DETEKCE PRO KATEGORIE
// ============================================================
...
// ============================================================
// UNIVERZÁLNÍ GALERIE PRO BLOG A PORTFOLIO (GLOBÁLNÍ)
// ============================================================

window.openImageGallery = function(images, startIndex = 0, type = 'blog') {
    if (!images || images.length === 0) {
        console.warn('openImageGallery: žádné obrázky');
        return;
    }
    
    console.log('🎬 Otevírám galerii s', images.length, 'obrázky, typ:', type);
    ...
    if (!gal || !galImg) {
        console.error('Galerie elementy nenalezeny');
        return;
    }
