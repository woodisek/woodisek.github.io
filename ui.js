...
// ============================================================
// MEMOIZACE FILTROVÁNÍ (CACHE VÝSLEDKŮ)
// ============================================================
...
// ============================================================
// SPRÁVA VIDITELNOSTI TLAČÍTEK BLOG A ABOUT
// ============================================================
...
export function updateNavButtonsVisibility() {
    ...
    // Blog a About - schovat podle nastavení
    if (blogBtn) {
    ...
    // Obchod - vždy viditelný a vždy klikací, ale změníme chování
    if (productsBtn) {
        if (CONFIG.Enable_Shop === false) {
            // Jen změníme vzhled (neprůsvitný, normální kurzor)
            productsBtn.style.opacity = '1';
            productsBtn.style.cursor = 'pointer';
            productsBtn.title = 'Obchod je dočasně zavřený';
            // Můžeš přidat malý symbol zavřeno
    ...
    card.innerHTML = `
        <div class="card-front">
            <div class="img-box" data-action="open-gallery" data-idx="${p.idx}">
                ...
                <img src="${p.imgs[0]}" loading="lazy" class="main-img" id="img-${p.idx}" onload="window.hideLoad(${p.idx})">
                ${p.imgs.length > 1 ? `
...
function updateProductCount() {
    ...
    // ZKONTROLUJTE, ZDA JE NĚJAKÝ PRODUKT VIDITELNÝ V DOMU
    ...
    // Pokud nejsou žádné viditelné produkty v DOMu
    if (!hasVisibleProducts && allFilteredProducts && allFilteredProducts.length === 0) {
        productCountEl.style.visibility = 'hidden';
        productCountEl.style.display = 'block';
        productCountEl.innerHTML = '&nbsp;';
        return;
    }
    
    // Pokud ještě nejsou produkty načteny
    if (!allFilteredProducts || allFilteredProducts.length === 0) {
        productCountEl.innerHTML = CONFIG.Product_Loading_Text || "🪵 Načítání dřeva...";
        productCountEl.style.visibility = 'visible';
        productCountEl.style.display = 'block';
        return;
    }
    ...
    if (currentCategory === 'Vše') {
        productCountEl.innerHTML = `${total} ${CONFIG.Product_Count_Text || "unikátních kousků"}`;
    } else {
        productCountEl.innerHTML = `${CONFIG.Product_Category_Text || "🪵 v kategorii "}${currentCategory}`;
    }
}
...
// V ui.js, upravte observer callback:
...
// Zobrazení blogu místo produktů (schová vše nepotřebné)
// Zobrazení blogu
...
// Zobrazení produktů (výchozí – vše vidět)
